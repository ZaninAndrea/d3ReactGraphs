...
class Graph extends Component {
  render() {
    const { maxPoints = 10, data, threshold } = this.props;
    ...
    let parseSlice = slice =>
      slice.reduce(
        (acc, el) => [
          ...acc,
          ...(el[2] ? [el] : [[el[0], el[1], false], [el[0], el[1], true]])
        ],
        []
      );
    ...
    return (
      <svg height="230" width="420" fill="none" stroke="black">
        <defs>
          <clipPath id="graphBody">
            <rect
              x="30"
              y={Math.max(0, scaleY(threshold))}
              width="385"
              height={210 - Math.max(0, scaleY(threshold))}
            />
          </clipPath>
          <clipPath id="graphThresholdBody">
            <rect x="30" y="0" width="385" height={scaleY(threshold)} />
          </clipPath>
        </defs>
